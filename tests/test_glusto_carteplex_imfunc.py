...
"""Test im_func with the carteplex class clones.
   Run with:
    glusto --pytest='-vv tests/test_glusto_carteplex_imfunc.py --capture=no'
"""
from glusto_base_class import GlustoBaseClass
from glusto_base_class import shoots_with

import pytest


@shoots_with([['olympus', 'fuji', 'olympus'],
              ['35mm', '85mm'],
              ['none', 'polarizer']])
class MyGlustoTest(GlustoBaseClass):
    ...
    def setUpClass(cls):
        print("Shoots with %s camera and %s lens and %s filter" % (cls.camera,
                                                                   cls.lens,
                                                                   cls.filter))
        GlustoBaseClass.setUpClass.im_func(cls)
    ...
    def setUp(self):
        ...
        GlustoBaseClass.setUp.im_func(self)
        '''
    ...
    def test_glusto1(self):
        ...
        print("\t\t%s with lens %s" % (self.camera, self.lens))
    ...
    def tearDownClass(cls):
        ...
        GlustoBaseClass.tearDownClass.im_func(cls)
        print("**Class name**: ", cls.__name__)
        print("***Class mro**: ", cls.__mro__)
        print("***Class bases**: ", cls.__bases__)
        print("***Class super**: ", cls.__mro__[1])
